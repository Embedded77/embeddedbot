...
let db = new sqlite3.Database('hello');
function update(arg1){
  db.run(`INSERT INTO money VALUES (?),(?)`,arg1, function(err) {
    if (err) {
...
message.channel.send(covidembed)
  ...
  fetch('https://api.covid19api.com/summary')
    ...
    .then(json => {
      covidembed=new discord.RichEmbed()
      covidembed.setTitle("Global Coronavirus Cases")
      var pickedcity = lodash(json,o => o.CountryCode ==args[1]||o.Country ==args[1]);
if (pickedcity==undefined){
return message.channel.send("this is not a valid country code or name!")

}
      for(var attributename in pickedcity){
        covidembed.addField(attributename,commas(pickedcity[attributename]))
}
message.channel.send(covidembed)
    })
    .catch(err => {
